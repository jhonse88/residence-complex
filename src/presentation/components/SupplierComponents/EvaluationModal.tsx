...
import { SupplierResponseDto } from '../../../application/dto/SupplierDto'
...
const EvaluationModal = ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size='md' onCloseComplete={() => setIsEditing(false)}>
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>{isEditing ? 'Editar Calificación' : 'Calificación del Servicio'}</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          {isFetching ? (
            ...
            <>
              <Box mb={4}>
                ...
                {isEditing ? (
                  <HStack spacing={1}>
                    {[1, 2, 3, 4, 5].map(star => (
                      <Icon
                        as={star <= (hoverRating || rating) ? HiStar : HiOutlineStar}
                        key={star}
                        boxSize={6}
                        color={star <= (hoverRating || rating) ? starColor : 'gray.300'}
                        cursor='pointer'
              ...
              <Box mb={4}>
                ...
                {isEditing ? (
                  ...
                  <Box p={3} borderWidth='1px' borderRadius='md' borderColor='gray.200' minH='100px'>
                    {comments ? (
                      <ChakraText whiteSpace='pre-wrap'>{comments}</ChakraText>
                    ) : (
                      <ChakraText color='gray.500'>No hay comentarios</ChakraText>
                    )}
                  </Box>
                )}
              </Box>
        ...
        <ModalFooter>
          {isEditing ? (
            <>
              <Button variant='outline' mr={3} onClick={handleCancelEditing} isDisabled={isLoading}>
                Cancelar
              </Button>
              <Button colorScheme='blue' onClick={handleSaveEvaluation} isLoading={isLoading} isDisabled={rating === 0}>
                Guardar Cambios
              </Button>
            </>
          ) : (

...
export default function SupplierTable() {
  ...
  const [supplier, setSupplier] = useState<SupplierResponseDto>({
    id: 0,
    name: '',
    phone: '',
    email: '',
      ...
      <Box px={10}>
        ...
        <TableContainer>
          <Table variant='striped' colorScheme='teal'>
            ...
            <Tbody>
              {suppliers.length > 0 ? (
                suppliers.map((supplier: SupplierResponseDto) => (
                  <Tr key={supplier.id}>
                    <Td>{supplier.id}</Td>
                    <Td>{supplier.name}</Td>
                    ...
                    <Td>
                      {supplier.averageRating ? (
                        <Flex align='center'>
                          <Text mr={2}>{supplier.averageRating.toFixed(1)}/5</Text>
                          <Flex>
                            {[...Array(5)].map((_, i) => {
                              const starValue = i + 1
                              ...
                              if (rating >= starValue) {
                                return <Icon as={FaStar} key={`${supplier.id}-star-${i}`} color='yellow.400' />
                              } else if (rating >= starValue - 0.5) {
                                return <Icon as={FaStarHalfAlt} key={`${supplier.id}-star-${i}`} color='yellow.400' />
                              } else {
                                return <Icon as={FaRegStar} key={`${supplier.id}-star-${i}`} color='yellow.400' />
                              }
                            })}
                          </Flex>
                        </Flex>
                    ...
                    <Td>
                      <Flex>
                        <IconButton
                          ...
                          border='none'
                          bg='transparent'
                          onClick={() => handleOpenModalAndDeleteConfirmation(`${supplier.id}`)}

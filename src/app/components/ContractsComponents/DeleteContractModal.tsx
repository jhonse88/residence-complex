...
interface Props {
  isOpen: boolean
  onClose: () => void
  contractIdToDelete: number // Cambiado el nombre para ser más explícito
  onDelete: () => void
}
...
const DeleteContractModal: FC<Props> = ({
  isOpen,
  onClose,
  contractIdToDelete, // Cambiado el nombre
  onDelete
}) => {
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete('/api/contracts', {
        data: { Id: contractIdToDelete } // Enviar en el cuerpo
      })
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.error || 'Ocurrió un error al eliminar el contrato',
        status: 'error',
        duration: 3000,
        isClosable: true
      })
